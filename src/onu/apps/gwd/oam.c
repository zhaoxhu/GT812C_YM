...
#endif
#ifdef HAVE_BOOT_ERASE_TEST_CMD
#include "../../cmn/plat/flash/flash_dev_driver.h"
#endif
	...
	const unsigned char SYS_SOFTWARE_MAJOR_VERSION_NO = 1;
	const unsigned char SYS_SOFTWARE_RELEASE_VERSION_NO = 1;
	const unsigned char SYS_SOFTWARE_BRANCH_VERSION_NO = 112;
	const unsigned char SYS_SOFTWARE_DEBUG_VERSION_NO = 1;
	...
	const unsigned char SYS_HARDWARE_DEBUG_VERSION_NO = 1;
#endif
#if (PRODUCT_CLASS == PRODUCTS_GT812C_B)
	const unsigned char SYS_SOFTWARE_MAJOR_VERSION_NO = 1;
	const unsigned char SYS_SOFTWARE_RELEASE_VERSION_NO = 1;
	const unsigned char SYS_SOFTWARE_BRANCH_VERSION_NO = 112;
	const unsigned char SYS_SOFTWARE_DEBUG_VERSION_NO = 1;
...
extern int oper_flash_erase_block(void* block);
extern int oper_flash_bulk_write(void* _addr, void* _data, int len);

int cmd_boot_erase_test(struct cli_def *cli, char *command, char *argv[], int argc)
{
    void *temp_ptr=0;
    void *test_addr = (void *)(BOOT_FLASH_ADDR_START);
        ...
        switch(argc)
        {
            case 1:
                ...
                "<cr>", "Just press enter to execute command!",
                 NULL);
                return cli_arg_help(cli, 0,
                "<addr>", "test addr 0x0x40000000 - 0x40010000",
                 NULL);
            default:
                return cli_arg_help(cli, argc > 1, NULL);
        }
    ...
    {
        cs_uint32 input_addr = iros_strtol(argv[0]);
        test_addr = (void*)input_addr;
       if( ( (unsigned long )test_addr < BOOT_FLASH_ADDR_START )
            ||( (unsigned long )test_addr >= BOOT_FLASH_ADDR_END))
        {
            cli_print(cli,"wrong addr\n");
            return CLI_OK;
        }
    }
    else
    ...
    oper_flash_erase_block(test_addr);
    
    temp_ptr=iros_malloc(IROS_MID_SYS, TEST_DATA_LEN);
    if(!temp_ptr){
        cli_print(cli,"cmd_boot_erase_test malloc failed\n");
        return CLI_OK;
    }
    memset(temp_ptr,0,TEST_DATA_LEN);

    oper_flash_bulk_write(test_addr,temp_ptr,TEST_DATA_LEN);
    
    return CLI_OK;
}
#endif /* #ifdef HAVE_BOOT_ERASE_TEST_CMD */
...
//		mtodo: cmd_oam_port_isolate modify
	...
	#endif
	#ifdef HAVE_BOOT_ERASE_TEST_CMD
        cli_register_command(cmd_root, NULL, "boot_erase_test", cmd_boot_erase_test, PRIVILEGE_PRIVILEGED, MODE_DEBUG, "Try to destroy boot partition!!!This may cause ONU CAN NOT bootup");
        #endif
    return;
}
...
#endif
#define ARP_MONITOR_INTERVAL_TIME	1000	//����
#define ARP_INTO_CPU_RATE_THRESHOLD	100		//ÿ��100��
#define ARP_INTO_CPU_DELAY_PERIOD 	10		//arp ������ʱ����cpu ������
