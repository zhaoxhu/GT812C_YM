...
/* check if erase/write addr point to boot partiton */
int boot_addr_check(void *addr)
{
    if(NULL == addr)
    {
        return -1;
    ...
    if( (BOOT_FLASH_ADDR_START <= (unsigned long )addr) && ( (unsigned long )addr < BOOT_FLASH_ADDR_END))
    {
        cs_printf("addr %p is in boot partition [0x%x,0x%x)!!!\n",addr,BOOT_FLASH_ADDR_START,BOOT_FLASH_ADDR_END);
        return -1;
    }
...
#define BOOT_PROTECT(addr)                                                           \
    if(0 != boot_addr_check(addr))                                                       \
    {                                                                                                            \
        cs_printf(" forbid to erase/write boot partition\n");                 \
        return -1;                                                                                        \
    }               
